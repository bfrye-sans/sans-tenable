...
class tenable::agent (
  ...
  exec { 'get_nessus_agent_version':
    command   => '/bin/bash -c "if command -v /opt/nessus_agent/sbin/nessuscli > /dev/null 2>&1; then /opt/nessus_agent/sbin/nessuscli -v | sed -n \"s/.*Nessus Agent) \\([0-9]\\+\\.[0-9]\\+\\.[0-9]\\+\\).*/\\1/p\" > /opt/puppetlabs/facter/facts.d/nessus_version.txt; else echo \"nessus_version=0.0.0\" > /opt/puppetlabs/facter/facts.d/nessus_version.txt; fi"',
